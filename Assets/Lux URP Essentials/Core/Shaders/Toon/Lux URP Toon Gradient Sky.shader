...
Shader "Lux URP/Gradient Sky"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                half gradient = saturate(dot( normalize(float3(input.uv.xyz)), input.dir) * 0.5h + 0.5h);
                half4 col = lerp(_GroundColor, _TopColor, pow(abs(gradient), _FallOff)) * _Intensity;
