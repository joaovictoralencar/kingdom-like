...
//  "uv" only exists in Varyings under the very same condition, so the debug macro has to
//  be guarded as well - otherwise DEBUG_DISPLAY breaks on materials without any texture.
    #if defined(_TEXMODE_ONE) || defined(_TEXMODE_TWO) || defined(_TEXMODE_TWO) || defined(_NORMALMAP) || defined(_MASKMAP)
