...
//  NOTE: Do NOT force define ADDITIONAL_LIGHT_CALCULATE_SHADOWS here.
//  The Inputs include above already pulled in Lighting.hlsl -> Shadows.hlsl, so defining it
//  at this point is too late to make URP declare _AdditionalLightsWorldToShadow and friends.
//  All it did was defeat the runtime guard in LuxAdditionalLightRealtimeShadow below, which
//  then compiled against arrays that do not exist - e.g. with Additional Lights set to
//  "Per Vertex", where URP does not enable _ADDITIONAL_LIGHT_SHADOWS at all.
...
real LuxSampleShadowmap(TEXTURE2D_SHADOW_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData, half4 shadowParams, bool isPerspectiveProjection = true)
{
    ...
    {
    //  Needs #else, not #endif: without ADDITIONAL_LIGHT_CALCULATE_SHADOWS the arrays below
    //  (_AdditionalLightsWorldToShadow, _AdditionalShadowParams, _AdditionalLightsShadowmapTexture)
    //  are not declared by Shadows.hlsl at all, so the body must be excluded, not just skipped.
    ...
    //  New sampler since URP 14.0.7
        ...
        #endif // ADDITIONAL_LIGHT_CALCULATE_SHADOWS
    }
#endif
...
//  Early out
    ...
    half3 normalTS = Lux_SampleNormalTS(uv, blendUv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap));
...
//  We do not use the macro here
    ...
    half alpha = albedo.a;
        ...
        half3 emission = Lux_BlendTexture(TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap), uv, blendUv).rgb * _EmissionColor.rgb;
    #else
