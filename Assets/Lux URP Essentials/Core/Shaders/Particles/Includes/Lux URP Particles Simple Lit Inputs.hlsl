...
//  ----------------------------------------------------------------------------------
//  Lux local copies of URP's particle texture helpers.
//  URP changed BlendTexture()/SampleNormalTS() from a separate texture+sampler pair to a
//  bundled UnityTexture2D struct (still the old form in 17.3, the new one in 17.7), so
//  calling URP's version directly breaks on one version or the other. Own names, own
//  signatures - no dependency on the URP version at all.

half4 Lux_BlendTexture(TEXTURE2D_PARAM(blendTex, sampler_blendTex), float2 uv, float3 blendUv)
{
    half4 color = half4(SAMPLE_TEXTURE2D(blendTex, sampler_blendTex, uv));
    #ifdef _FLIPBOOKBLENDING_ON
        half4 color2 = half4(SAMPLE_TEXTURE2D(blendTex, sampler_blendTex, blendUv.xy));
        color = lerp(color, color2, half(blendUv.z));
    #endif
    return color;
}

half3 Lux_SampleNormalTS(float2 uv, float3 blendUv, TEXTURE2D_PARAM(bumpMap, sampler_bumpMap), half scale = half(1.0))
{
    #if defined(_NORMALMAP)
        half4 n = Lux_BlendTexture(TEXTURE2D_ARGS(bumpMap, sampler_bumpMap), uv, blendUv);
...
{
    half4 albedo = Lux_BlendTexture(TEXTURE2D_ARGS(albedoMap, sampler_albedoMap), uv, blendUv) * color;
...
#ifdef _SPECGLOSSMAP
    specularGloss = Lux_BlendTexture(TEXTURE2D_ARGS(specGlossMap, sampler_specGlossMap), uv, blendUv);
