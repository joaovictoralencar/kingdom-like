...
// Deferred optimization using "UniversalMaterialType" = "Unlit"

Shader "Lux URP/Cloth STL"
{
    Properties
    {
        ...
        _MaskMap                    ("     Occlusion (G) Thickness (B) Smoothness (A)", 2D) = "white" {}
...
// Using Unlit here will setup the stencil buffer in the GBuffer pass,
// so that deferred lighting will skip the related pixels
// NOTE: Must be assigned here and not for the GBuffer pass only!
"UniversalMaterialType" = "Unlit"
    ...
    //  Shadows -----------------------------------------------------
        ...
        //  Include base inputs and all other needed "base" includes
            ...
            #include "Includes/Lux URP Cloth STL ShadowCaster Pass.hlsl"
    ...
    //  GBuffer Pass - minimalized which only outputs depth and writes into the normal buffers  
        ...
        //  As we do not store the alpha mask with the base map we have to use custom functions 
            ...
            GBufferFragOutput LitGBufferPassFragment(Varyings input, half facing : VFACE)
            {
                ...
                half3 packedNormalWS = PackGBufferNormal(input.normalWS);
    ...
    //  Meta -----------------------------------------------------
        ...
        //  First include all our custom stuff
            #include "Includes/Lux URP Cloth Inputs.hlsl"
            #include "Includes/Lux URP Cloth STL Meta Pass.hlsl"
...
Cull[_Cull]
    ...
    CustomEditor "LuxURPEssentials.LuxURPUniversalCustomShaderGUI"
}
