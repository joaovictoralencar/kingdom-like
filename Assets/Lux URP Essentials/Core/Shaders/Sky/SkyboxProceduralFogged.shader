...
Shader "Lux URP/Skybox/Procedural" {
Properties {
    [KeywordEnum(None, Simple, High Quality)]
    _SunDisk                ("Sun", Int) = 2
    ...
    [Header(Fog)]
    [Space(10)]
    _FogStart               ("Fog Start", Float) = -6
    _FogEnd                 ("Fog End", Float) = 30
    _FogDensity             ("Fog Density", Range(0, 1)) = 1
    [Toggle]
    _EnableScattering       ("Enable Scattering", Float) = 0
    _ScatteringSize         ("     Scattering Size (inverse)", Range(2, 20)) = 4
    _ScatteringTint         ("     Scattering Tint", Color) = (.5, .5, .5, .5)
}
    ...
    SubShader {
        ...
        Pass {
            ...
            half _FogStart, _FogEnd, _FogDensity, _EnableScattering, _ScatteringSize;
    ...
    //              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            if(_FogDensity > 0)
            {
                half3 fogColor = unity_FogColor;
                #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
                    if(_EnableScattering)
                    {
                        half focusedEyeCos = pow(saturate(dot(_WorldSpaceLightPos0.xyz, -ray)), _ScatteringSize );
                        //half Mie = getMiePhase(-focusedEyeCos, focusedEyeCos * focusedEyeCos);
                        fogColor *= 1.0 + _ScatteringTint.rgb * unity_ColorSpaceDouble.rgb * focusedEyeCos;   
                    }
                #endif
                col = lerp(col, fogColor, _FogDensity * saturate( smoothstep(-_FogEnd, _FogStart, y)));
            }
