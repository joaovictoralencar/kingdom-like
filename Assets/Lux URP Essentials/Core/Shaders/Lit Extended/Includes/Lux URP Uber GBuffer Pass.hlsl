...
half3 LuxExtended_GlobalIllumination(BRDFData brdfData, half3 bakedGI, half occlusion, float3 positionWS, half3 normalWS, half3 viewDirectionWS, half3 bentNormal, half3 geoNormalWS, half horizonOcllusion, float2 normalizedScreenSpaceUV)
{
...
//  Horizon Occlusion
    ...
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfData.perceptualRoughness, 1.0h, normalizedScreenSpaceUV ) * reflOcclusion * occlusion;

    return EnvironmentBRDF(brdfData, indirectDiffuse, indirectSpecular, fresnelTerm);
}

// Used in Standard (Physically Based) shader
GBufferFragOutput LitGBufferPassFragment(Varyings input, half facing : VFACE)
{
    UNITY_SETUP_INSTANCE_ID(input);
...
//  Calculate bitangent upfront
    ...
    //  Screen space irradiance is a screen space buffer, so it can not be re-sampled along the bent normal.
        #if !defined(LIGHTMAP_ON) && !defined(DYNAMICLIGHTMAP_ON) && !defined(_SCREEN_SPACE_IRRADIANCE)
    ...
    half3 color = LuxExtended_GlobalIllumination(brdfData, inputData.bakedGI, surfaceData.occlusion, inputData.positionWS, inputData.normalWS, inputData.viewDirectionWS,
            ...
            half3(0,0,0),
        #endif
        input.normalWS, _HorizonOcclusion, inputData.normalizedScreenSpaceUV );
