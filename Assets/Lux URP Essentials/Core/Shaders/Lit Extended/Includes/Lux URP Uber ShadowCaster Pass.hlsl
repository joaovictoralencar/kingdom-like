...
//  When rendering backfaces normal extrusion is in the wrong direction: the authored
//  normal points away from the light, so ApplyShadowBias picks invNdotL = 1 (its maximum
//  normal bias) and applies it along -light - pushing the caster into the surface and
//  making concave areas shadow themselves. So orient the normal towards the light.
//  NOTE: The earlier attempt used sign(dot(normal, lightDir)) which returns 0 at exactly
//  grazing incidence and thus cancelled the normal bias completely right where it peaks.
    float3 shadowNormalWS = normalInput.normalWS;
    if (dot(shadowNormalWS, lightDirectionWS) < 0.0)
        shadowNormalWS = -shadowNormalWS;

    output.positionCS = TransformWorldToHClip(ApplyShadowBias(positionWS, shadowNormalWS, lightDirectionWS));
