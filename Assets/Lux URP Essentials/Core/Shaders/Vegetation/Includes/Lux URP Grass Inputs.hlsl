...
//  DOTS - we only define a minimal set here. The user might extend it to whatever is needed.
    ...
    void BendGrassReal(float3 positionOS, half3 normalOS, half4 vertexColors, float4 timeParams, Texture2D windTex, SamplerState windTexSampler, out float3 positionWS, out half3 normalWS, out half2 fadeOcclusion) {
        ...
        //  Mind spatial coherency!
            half4 wind = SAMPLE_TEXTURE2D_LOD(windTex, windTexSampler, (positionWS.xz + phase * _WindMultiplier.z) * _LuxURPWindDirSize.w, _WindMultiplier.w);
    ...
    //  Add small scale jitter (Horizon Zero Dawn)
        ...
        float3 disp = sin( 4.0f * 2.650f * (positionWS.x + positionWS.y + positionWS.z + timeParams.x)) * normalWS * float3(1.0f, 0.35f, 1.0f);
...
//  Old signature - but we have to add time as input
    ...
    {
        BendGrassReal(positionOS, normalOS, vertexColors, _TimeParameters, _LuxURPWindRT, sampler_LuxURPWindRT, positionWS, normalWS, fadeOcclusion);
    }
