...
//  These have to be zero initialized, otherwise decals and debug error!
    ...
    half4 t_shadowMask = (half4)0.0;
        ...
        bakedGI = SAMPLE_GI(
            ...
            normalWS,
            viewDirectionWS,
            normalizedScreenSpaceUV * _ScreenSize.xy,           //positionSPRaw.xy * rcp(positionSPRaw.w) * GetScaledScreenParams().xy, // not bad, but not stable
            ProbeOcclusion, 
            t_shadowMask
        );
    #else
...
//  Fill standard URP structs so we can use the built in functions
    InputData inputData = (InputData)0;
    {
        ...
        inputData.bakedGI = bakedGI;
            ...
            inputData.shadowCoord = ComputeScreenPos(positionCS); //Here we needed raw: inputData.shadowCoord = positionSPRaw;
        #else
...
//  From here on we rely on surfaceData and inputData only!
